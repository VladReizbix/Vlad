    ...
    match table_name:
        case 'Житель': 
            ...
            return (
                ...
                f"Текущий коэффициент: {row['Текущий коэффициент'].fillna(1.0).values[0]:.1f}\n"
                f"Количество списанных Ростиков: {int(row['Количество списанных Ростиков'].fillna(0).values[0])}\n"
                f"Количество полученных призов: {int(row['Количество полученных призов'].fillna(0).values[0])}"
            )
        case 'НКО':
            photo_path = './2л.jpg'  
            with open(photo_path, 'rb') as photo:
        ...
        case 'Школа':
            ...
            return (
                ...
                f"Название организаций: {row['Название организации'].values[0]}\n"
                f"Общее количество выполненных мероприятий: {int(row['Общее количество Мероприятий'].fillna(0).values[0])}\n"
                f"Общее количество активных участников на мероприятиях(Добрые Дела): {int(row['Общее количество Добрых дел'].fillna(0).values[0])}\n"
                f"Город: {row['Глобальный субъект'].values[0]}"
            )
